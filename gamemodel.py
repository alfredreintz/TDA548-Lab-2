...
class Player:
    ...
    def fire(self, angle, velocity):
        ...
        if currentPlayer == self.gameObj.getPlayers()[1]:
            angle = 180 - angle

        fireProjectile = Projectile(
            angle,
            velocity,
            self.gameObj.getCurrentWind(),
            self.getX(),
            self.gameObj.getCannonSize() / 2,
            -110,
            110,
        )
    ...
    def projectileDistance(self, proj):
        # If the cannon and the projectile intersects
        if abs(proj.getX() - self.getX()) <= self.gameObj.getCannonSize() / 2 + self.gameObj.getBallSize():
            return 0
        elif proj.getX() > self.getX():
            # The distance between the edges of the cannon and projectile
